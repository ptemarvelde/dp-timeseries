...
RESULT_DIR = '../../resources/gs-wgan/results'


def parse_arguments():
    parser = argparse.ArgumentParser()
    parser.add_argument('--random_seed', '-s', type=int, default=1, help='random seed')
    parser.add_argument('--dataset', '-data', type=str, default='mnist', choices=['stock', 'sine', 'mnist', 'fashionmnist', 'mitbih', 'ptb'],
                        help=' dataset name')
    parser.add_argument('--num_discriminators', '-ndis', type=int, default=10, help='number of discriminators')
    ...
    parser.add_argument('--pretrain_download', '-predl', type=bool, help='Download pretrained discriminators from google drive')
    ...
    parser.add_argument('--save_iterations', '-eps', type=float, nargs='+', default=[],
                        help='Treat the iterations at these epsilon values as save steps')
