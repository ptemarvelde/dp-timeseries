...
def main(args):
    ...
    ### Data loaders
    trainset, NUM_CLASSES, width, height, classes_split = load_dataset(dataset_name, DATA_ROOT, save_dir=False)
    print(f"trainset {trainset}, {NUM_CLASSES}, {width}")
    IMG_DIM = width * height
    samples_dim = (width, height)
    SAMPLES_PER_BATCH = NUM_CLASSES
    ...
    for idx, netD_id in enumerate(net_ids):
        print(f"Running {netD_id} of total {num_discriminators}")
        ...
        if os.path.exists(save_subdir):
            ...
            for iter in range(args.pretrain_iterations + 1):
                ...
                if iter < 5 or iter % args.print_step == 0:
                    ...
                    print('Step: {}, G_cost:{}, D_cost:{}, Wasserstein:{}, Time since previous print {} s'.format(iter, G_cost.cpu().data,
                                                                        D_cost.cpu().data,
                                                                        Wasserstein_D.cpu().data,
                                                                        round(seconds)
                                                                        ))
                if iter == args.pretrain_iterations:
                    generate_sample(dataset_name, iter, netG, fix_noise, save_subdir, device,
                                    num_classes=NUM_CLASSES, img_w=samples_dim[0], img_h=samples_dim[1])
