...
# inception does not work
# MNIST_MODULE = 'https://tfhub.dev/tensorflow/tfgan/eval/inception/1'
MNIST_MODULE = 'https://tfhub.dev/tensorflow/tfgan/eval/mnist/logits/1'
FASHION_MNIST_MODULE = "../data/mnist_models/fashionmnist/CNN/"


def get_classifier():
    return tfhub.load(MNIST_MODULE)


def mnist_score(images, classifier, dataset='mnist', num_batches=1):
  ...
  """
    ...
    if dataset == 'mnist':
        score = tfgan.eval.classifier_score(images, classifier, num_batches)
    elif dataset == 'fashionmnist':
        probs = classifier.predict(images, verbose=1, steps=1)
        score = tfgan.eval.classifier_metrics.classifier_score_from_logits(probs)
    else:
        raise ValueError(f"{dataset} is not a valid dataset for mnist score")
