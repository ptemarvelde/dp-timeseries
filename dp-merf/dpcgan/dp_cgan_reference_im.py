...
baseDir = "../../resources/dp-cgan-new/fashionmnist/run3"
sys.path.append(baseDir)
# sys.path.append("/main/dpcgan/")

from differential_privacy.dp_sgd.dp_optimizer import dp_optimizer, utils, sanitizer
from differential_privacy.privacy_accountant.tf import accountant
from differential_privacy.privacy_accountant.python.rdp_accountant import compute_rdp, get_privacy_spent
...
def runTensorFlow(sigma, clipping_value, batch_size, epsilon, delta, iteration, data_save_str,
    ...
    if dataset_key == 'digits':
        mnist = input_data.read_data_sets("../../resources/data/MNIST/raw", one_hot=True)
    elif dataset_key == 'fashion':
        print('using FashionMNIST')
        mnist = input_data.read_data_sets("../../resources/data/FashionMNIST/FashionMNIST/raw", one_hot=True)
    else:
        raise ValueError
    ...
    result_dir = baseDir
    if not os.path.exists(result_dir+"/gen"):
        os.makedirs(result_dir+"/gen")
    ...
    tempsamples = mnist.train.images[:10]
    fig = plot(tempsamples)
    plt.savefig(
        (result_path + "/train_images.png"), bbox_inches='tight')
    plt.close(fig)
    ...
    # Main Session
    losses = ["step, d_loss_real, d_loss_fake, g_loss\n"]
    with tf.Session(config=tf.ConfigProto(allow_soft_placement=True, gpu_options=gpu_options)) as sess:
        ...
        while step <= num_training_steps and should_terminate is False:
            ...
            if step % 50 == 0:
                losses.append(f"{step}, {d_loss_real_curr}, {d_loss_fake_curr}, {g_loss_curr}\n")
                print("step :  " + str(step) + "  eps : " + str(eps) + "\t" + losses[-1], end="")
            ...
            if step % save_step == 0:
                ...
                loc = f'{save_str}gen/eps{max_target_eps}_{step}.npz'
                np.savez(loc, data=images, labels=image_labels)
                print(f'done saving to {loc}')
...
def main():
    ...
    data_save_loc = "../../resources/dp-cgan-new/fashionmnist/run3/"
    data = 'fashion'
    ...
    for sigma, clipping in sigma_clipping_list:
        for batchSize in batchSizeList:
            ...
            runTensorFlow(sigma, float(clipping), batchSize, epsilon, delta, iteration, data_save_loc,
                          data, run_auc_test=False)
